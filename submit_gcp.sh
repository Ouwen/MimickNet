...
JOB_NAME=MimickNet_Blackbox_$(date +%Y_%m_%d_%H%M%S)
...
gcloud ai-platform jobs submit training $JOB_NAME \
  ...
  --train_clinical_csv 'gs://duke-research-us/mimicknet/data/training_b-v2.csv'
...
JOB_NAME=MimickNet_Blackbox_VerasonicsOnly$(date +%Y_%m_%d_%H%M%S)
JOB_DIR=$GCS_BUCKET"/"$JOB_NAME
gcloud ai-platform jobs submit training $JOB_NAME \
  --master-image-uri $IMAGE_URI \
  --scale-tier custom \
  ...
  --job-dir $JOB_DIR \
  -- \
  --train_das_csv 'gs://duke-research-us/mimicknet/data/training-v2-verasonics_a.csv' \
  --train_clinical_csv 'gs://duke-research-us/mimicknet/data/training-v2-verasonics_b.csv'
gcloud ai-platform jobs describe $JOB_NAME

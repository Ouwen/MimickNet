...
def main(argv):
    ...
    model.fit(train_dataset,
              ...
              verbose=1,
              callbacks=[log_code, terminate, tensorboard, saving, reduce_lr, copy_keras, image_gen, early_stop, get_csv])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--m',        default= True, type=bool, help='denotes a manual run or hp tuning')
    ...
    parser.add_argument('--l_ssim',   default=1, type=float, help='ssim loss')
    parser.add_argument('--l_mae',    default=0, type=float, help='mae loss')
    parser.add_argument('--l_mse',    default=0, type=float, help='mse loss')

    # Cloud ML Params
    parser.add_argument('--job-dir', default='gs://duke-research-us/mimicknet/experiments/unet/debug/{}'.format(str(time.time())), help='Job directory for Google Cloud ML')
